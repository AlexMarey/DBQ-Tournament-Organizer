...
exports.addParticipantToTournament = (challonge_url, participant, message) => {
    if (challonge_url) {
        if(participant.name) {
            ...
            return `I didn't catch who wanted to sign up. Try again or use a different command.`;
        }
    } else {
        return `Unsuccesfully connected to challonge. Check the tournament url and try again.`;
    }
};
